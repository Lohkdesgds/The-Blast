		...
		namespace Stacker {
			...
			const bool extractor::interpret(const std::string path)
			{
				...
				if (fopen_s(&fp, path.c_str(), "rb") != 0) {
					logg << Log::ERRDV << Log::START << "[EXTR:INTRP][ERRR] Failed on extractor::interpret = no file found / file is null." << Log::ENDL;
					return false;
				}
				...
				{
					logg << Log::ERRDV << Log::START << "[EXTR:INTRP][ERRR] Failed on extractor::interpret = This file is not supported!" << Log::ENDL;
					return false;
				}
				...
				if (amountofd <= 0) {
					logg << Log::ERRDV << Log::START << "[EXTR:INTRP][ERRR] Failed on extractor::interpret = Amount of files detected not valid!" << Log::ENDL;
					return false;
				}
				...
				for (__int64 u = 0; u < amountofd; u++)
				{
					...
					if (sa.filename.length() == 0) {
						logg << Log::ERRDV << Log::START << "[EXTR:INTRP][ERRR] Failed on extractor::interpret = One file read was bad formatted!" << Log::ENDL;
						return false;
					}
			...
			const bool extractor::extractAll(std::string at)
			{
				...
				if (fopen_s(&fp, orig.c_str(), "rb") != 0) {
					logg << Log::ERRDV << Log::START << "[EXTR:EXALL][ERRR] Failed on extractor::extractAll = Could not re-open file / file is null!" << Log::ENDL;
					return false;
				}
				...
				while (std::string(line) != "%LSW_E") {
					...
					if (feof(fp)) {
						logg << Log::ERRDV << Log::START << "[EXTR:EXALL][ERRR] Failed on compactor::extractAll = End of index not found / corrupted!" << Log::ENDL;
					}
				}
				...
				for (auto& i : stack) {
					...
					if (fopen_s(&fl, tmps.c_str(), "wb") != 0) {
						...
						if (fopen_s(&fl, tmps.c_str(), "wb") != 0) {
							...
							logg << Log::START << "[EXTR:EXALL][ERRR] Failed on compactor::extractAll = One of the files could not be created! Skipping..." << Log::ENDL;
							continue;
						}
					}
			...
			void compactor::insert(std::string nuev)
			{
				...
				if (sa.siz_o_file <= 0) {
					logg << Log::ERRDV << Log::START << "[COMP:INSRT][ERRR] Failed on compactor::insert = no file found / file is null." << Log::ENDL;
					return;
				}
				sa.filename = nuev;
				stack.push_back(sa);
			...
			void compactor::compactAll()
			{
				...
				if (fopen_s(&fp, orig.c_str(), "wb") != 0) {
					logg << Log::ERRDV << Log::START << "[COMP:CPALL][ERRR] Failed on compactor::compactAll = Cannot open the file for writing!" << Log::ENDL;
					return;
				}
				...
				for (auto& i : stack) {
					...
					if (fopen_s(&fl, i.filename.c_str(), "rb") != 0) {
						logg << Log::ERRDV << Log::START << "[COMP:CPALL][ERRR]Failed on compactor::compactAll = One of the files doesn't exist! (how?)" << Log::ENDL;
						return;
					}
