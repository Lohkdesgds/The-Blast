...
namespace LSW {
	...
	std::string LSW::GetSystemFolderPaths(int csidl)
	{
		wchar_t Folder[1024];
	...
	bool downloader::download(const char* singlefile, const char* to_where)
	{
		HINTERNET connect = InternetOpen((LPCSTR)connection_id.c_str(), INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
		if (!connect) return false;
	...
	bool downloader::downloadAsString(const char* singlefile, std::string& wheree)
	{
		HINTERNET connect = InternetOpen((LPCSTR)	connection_id.c_str(), INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
		if (!connect) return false;
		HINTERNET OpenAddress = InternetOpenUrlA(connect, singlefile, NULL, 0, INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_KEEP_CONNECTION, 0);
	...
	void processList(std::string alllines, float* perc)
	{
		...
		string gamespath = LSW::GetSystemFolderPaths(CSIDL_APPDATA) + "\\Lohk\'s Studios\\combolauncher";
